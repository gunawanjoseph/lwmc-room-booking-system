...
export function PublicBookings() {
  const approved=usePaginatedQuery(api.myBookings.publicList,{status:"approved"},{initialNumItems:20});
  const pending=usePaginatedQuery(api.myBookings.publicList,{status:"pending"},{initialNumItems:20});
  const settings=useQuery(api.myBookings.publicSettings,{});
  const [view,setView]=useState<"calendar"|"table">("calendar");
  const [statuses,setStatuses]=useState<string[]>(["approved","pending"]);
  ...
  useEffect(()=>{if(pending.status==="CanLoadMore")pending.loadMore(20);},[pending.status,pending.loadMore]);
  const all=useMemo(()=>[...approved.results,...pending.results].flatMap(row=>row.meetings),[approved.results,pending.results]);
  ...
  const rows=useMemo(()=>filterPublicMeetings(all,{statuses,ministries,rooms}),[all,statuses,ministries,rooms]);
  const timezone=settings?.timezone??"Asia/Singapore";
  const ready=approved.status==="Exhausted"&&pending.status==="Exhausted"&&settings;
  function reset(){setStatuses(["approved","pending"]);setMinistries([]);setRooms([]);setVisible(50);}
  return <main className="page my-bookings-page"><header className="my-bookings-header"><Link href="/"><Brand/></Link><Link className="button button-secondary" href="/sign-in">Administrator sign in</Link></header>
    <h1>Booking calendar</h1><p>See everyone's approved and pending room bookings. No account or sign-in needed.</p>
    <section className="panel my-bookings-panel">
      ...
      <p>{statuses.length===2&&!ministries.length&&!rooms.length?"All bookings (god mode) — approved and pending across every ministry and room.":"Filtered bookings"}</p>
      <div className="public-booking-filters"><FilterGroup label="Booking status" options={[{value:"approved",label:"Approved"},{value:"pending",label:"Pending"}]} selected={statuses} onChange={values=>{setStatuses(values);setVisible(50);}}/><FilterGroup label="Ministry" options={ministryOptions} selected={ministries} onChange={values=>{setMinistries(values);setVisible(50);}}/><FilterGroup label="Room" options={roomOptions} selected={rooms} onChange={values=>{setRooms(values);setVisible(50);}}/></div>
      <p className="public-filter-hint">Combine any filters. No ministry or room selected means all. Uncheck both statuses to hide all bookings. Pending requests are not confirmed reservations.</p>
