import { ministryDisplay } from "@/shared/requestFields";
...
export type ProposedEdit = { room: string; startAt: number; endAt: number; eventName: string; purpose: string; ministry: string; otherMinistry?: string; responses?: Array<{ qid: string; value: string }> };
export function BookingChangeComparison({ before, after, timezone }: { before: MeetingSummary; after: ProposedEdit; timezone: string }) {
  const fields = [
    ...
    ["Ministry", before.ministry || "—", ministryDisplay(after.ministry, after.otherMinistry) || "—"], ["Purpose", before.purpose || "—", after.purpose || "—"],
